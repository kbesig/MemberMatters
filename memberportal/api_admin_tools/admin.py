...
class SubscriptionAddonAdmin(admin.ModelAdmin):
    ...
    def create_stripe_objects(self, request, queryset):
        ...
        if success_count > 0:
            self.message_user(
                request,
                f"Successfully created Stripe objects for {success_count} add-on(s)",
            )

        if error_messages:
            self.message_user(
                request, f"Errors: {'; '.join(error_messages)}", level="ERROR"
            )
    ...
    def update_stripe_prices(self, request, queryset):
        ...
        if success_count > 0:
            self.message_user(
                request,
                f"Successfully updated Stripe prices for {success_count} add-on(s)",
            )
    ...
    def sync_all_addons(self, request, queryset):
        ...
        if success_count > 0:
            self.message_user(
                request, f"Successfully synced {success_count} add-on(s) with Stripe"
            )
    ...
    def save_model(self, request, obj, form, change):
        ...
        # Auto-sync with Stripe only when needed
        if not change:
            ...
            if not obj.stripe_product_id:
                ...
                if not success:
                    self.message_user(
                        request,
                        f"Warning: Could not create Stripe objects: {message}",
                        level="WARNING",
                    )
        elif form.changed_data:
            # Existing add-on with changes
            pricing_fields = ["cost", "interval", "interval_count", "currency"]
            if any(field in form.changed_data for field in pricing_fields):
                ...
                if obj.stripe_product_id:
                    ...
                    if not success:
                        self.message_user(
                            request,
                            f"Warning: Could not update Stripe price: {message}",
                            level="WARNING",
                        )
                else:
            ...
            elif "name" in form.changed_data or "description" in form.changed_data:
                # Name/description changed, update Stripe product
                if obj.stripe_product_id:
                    success, message = obj.update_stripe_product()
                    if not success:
                        self.message_user(
                            request,
                            f"Warning: Could not update Stripe product: {message}",
                            level="WARNING",
                        )
